// CRT and Bloom Combined Shader

// CRT Settings
float warp = 0.20; // CRT curvature effect
float scan = 0.30; // Scanline effect
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // *** CRT Effect - Warp distortion ***
    vec2 dc = abs(0.5 - uv);
    dc *= dc;
    uv.x -= 0.5; uv.x *= 1.0 + (dc.y * (0.1 * warp)); uv.x += 0.5;
    uv.y -= 0.5; uv.y *= 1.0 + (dc.x * (0.1 * warp)); uv.y += 0.5;
    ...
    for (int i = 0; i < 24; i++) {
        ...
        float l = lum(c);
        if (l > 0.2) { 
            color += l * s.z * c * 0.1; // Lowered bloom contribution
        }
    }

    // *** CRT Effect - Scanlines ***
    float scanline = abs(sin(fragCoord.y) * 0.25 * scan);
    color.rgb = mix(color.rgb, vec3(0.0), scanline); // Darken scanlines
